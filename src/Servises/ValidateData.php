...
class ValidateData
{
    ...
    public static function checkFormParams(array $data): array|bool
    {
        $_SESSION['formError'] = [];
        ...
        elseif (empty($data['email']) and !empty($data['password']))
        {
            ...
            $_SESSION['formError'] = 'Необходимо ввести email';            
        }
        elseif (!empty($data['email']) and empty($data['password']))
        {
            //dd('Поле password не заполнено');
            $_SESSION['formError'] = 'Необходимо ввести пароль';            
        }
        else
        {
            //dd('Поле email и password не заполнено');
            $_SESSION['formError'] = 'Необходимо ввести email и пароль';            
        }
    ...
    public static function validate(string $email, string $pass): bool
    {
        ...
        {
            $_SESSION['formError'] = 'Е-майл не корректен';
              return false;
        }        

        if ((strlen($pass) < 6 OR strlen($pass) > 12)) 
        {
            $_SESSION['formError'] = 'Пароль должен быть от 6 до 12 символов';
            return false;
        }
